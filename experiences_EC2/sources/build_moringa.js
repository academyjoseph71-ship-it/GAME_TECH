...
const EXP = JSON.parse(fs.readFileSync(path.join(__dirname, 'moringa.json')));
...
const SITUATIONS = {
  ...
  exp6: 'Au village, l’eau du marigot est si boueuse que les familles la laissent reposer toute une nuit dans la jarre, sans parvenir à la rendre claire. Une plante de la cour peut-elle aider à la clarifier et à la rendre plus sûre ?',
...
function table(title, head, rows, widthsCm, src, o = {}) {
  ...
  const cell = (t, j, hd) => new TableCell({
    ...
    children: [new Paragraph({ spacing: { line: 240 }, keepNext: true, alignment: hd ? AlignmentType.CENTER : AlignmentType.LEFT,
...
const cover = [
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 900, after: 200, line: 300 }, border: { top: box, bottom: box, left: box, right: box }, indent: { left: 300, right: 300 },
    children: runs('**CLARIFIER ET PURIFIER L’EAU AVEC LES GRAINES DE MORINGA**', { size: 30, color: '1F3864' }) }),
  c('Expérience pratique artisanale n° 6 : démarche, illustration et schéma directeur (Freeplane)', { before: 240, size: 26 }),
...
const intro = [
  ...
  para('Le chapitre 5 du support de cours de l’EC2 « Maladies et hygiène » présente trois procédés de traitement de l’eau : la sédimentation, la filtration et la désinfection. La présente fiche complète les cinq expériences déjà proposées par une sixième, qui renforce l’étape de sédimentation grâce à une ressource végétale très répandue au Togo : les graines de moringa (*Moringa oleifera*). Broyées, ces graines agissent comme un coagulant naturel qui rassemble la boue en flocons, lesquels tombent au fond du récipient. L’eau ainsi clarifiée est ensuite filtrée sur un pagne, puis désinfectée au soleil ou par ébullition.'),
  para('La fiche suit la même démarche d’investigation que les expériences précédentes : situation-problème, objectifs, matériel local, démarche pas à pas, observations, interprétation, sécurité et prolongements. Elle est complétée par une illustration du dispositif et par un schéma directeur réalisé dans Freeplane. Aucun achat n’est nécessaire.'),
];
const recap = [];
const introSuite = [];
...
const conclusion = [
  ...
  para('Cette expérience montre qu’une ressource végétale locale et gratuite, la graine de moringa, peut clarifier en une à deux heures une eau très boueuse que la simple décantation ne parvient pas à éclaircir. Elle illustre concrètement les étapes du traitement de l’eau étudiées au chapitre 5 et rappelle un principe essentiel : une eau claire n’est pas forcément potable. Associée à la filtration et à la désinfection solaire ou à l’ébullition, elle constitue un moyen simple de prévenir les maladies transmises par l’eau (choléra, amibiase, giardiase, fièvre typhoïde) dans les familles et les écoles.'),
  ...
  ...[
    ...
    'Folkard, G., et Sutherland, J. (1996). *Moringa oleifera*: A tree with enormous potential. *Agroforestry Today, 8*(3), 5-8.',
    'Ndabigengesere, A., et Narasiah, K. S. (1998). Quality of water treated by coagulation using *Moringa oleifera* seeds. *Water Research, 32*(3), 781-791.',
    'Organisation mondiale de la santé. (2017). *Directives de qualité pour l’eau de boisson* (4e éd.). OMS.',
...
const sections = [
  ...
  { properties: { page: { ...portrait, pageNumbers: { start: 1 } } }, footers: { default: footer }, children: intro },
];
EXP.forEach((e, i) => {
...
const doc = new Document({
  creator: 'ENFPE', title: 'Clarifier et purifier l’eau avec les graines de moringa – EC2 Maladies et hygiène', features: { updateFields: true },
  styles: {
...
Packer.toBuffer(doc).then(async (buf) => {
  ...
  fs.writeFileSync(path.join(__dirname, 'out/moringa_raw.docx'), await zip.generateAsync({ type: 'nodebuffer' }));
